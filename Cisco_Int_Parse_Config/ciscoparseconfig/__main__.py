...
def main():
    ...
    while True:
        user_input_str = input(
            'Paste text with the list of switch ports. Press Enter once again when done: ')
    ...
    gi_numeric_checker = r"\d+(?:\/\d+)?(?:\/\d{0,2})?$"
    te_numeric_checker = r"\d+(?:\/\d+)?(?:\/\d{0,2})?$"
    fa_numeric_checker = r"\d+(?:\/\d+)?(?:\/\d{0,2})?$"
    po_numeric_checker = r"\d{0,4}$"
    lo_numeric_checker = r"\d{0,4}$"
    eth_numeric_checker = r"\d+(?:\/\d+)?(?:\/\d{0,2})?$"
    ...
    for interface in interface_names:
        ...
        if re.search("gi", interface):
            ...
            if (f"gi{numeric_match}") not in interface_names_final:
                ...
                print(
                    f"Interface duplicate: ({(f'gi{numeric_match}')}). Please remove and start from beginning!")
                interface_names_final = []
                break
            ...
            if (f"te{numeric_match}") not in interface_names_final:
                ...
                print(
                    f"Interface duplicate: ({(f'te{numeric_match}')}). Please remove and start from beginning!")
                interface_names_final = []
                break
            ...
            if (f"fa{numeric_match}") not in interface_names_final:
                ...
                print(
                    f"Interface duplicate: ({(f'fa{numeric_match}')}). Please remove and start from beginning!")
                interface_names_final = []
                break
            ...
            if (f"lo{numeric_match}") not in interface_names_final:
                ...
                print(
                    f"Interface duplicate: ({(f'lo{numeric_match}')}). Please remove and start from beginning!")
                interface_names_final = []
                break
            ...
            if (f"po{numeric_match}") not in interface_names_final:
                ...
                print(
                    f"Interface duplicate: ({(f'po{numeric_match}')}). Please remove and start from beginning!")
                interface_names_final = []
                break
            ...
            if (f"eth{numeric_match}") not in interface_names_final:
                ...
                print(
                    f"Interface duplicate: ({(f'eth{numeric_match}')}). Please remove and start from beginning!")
                interface_names_final = []
                break
    ...
    if interface_names_final != []:
        ...
        while config_str != 'end':
            config_str = input(
                "Please enter desired interface configuration - Line by Line. Type 'end' at the last line to continue: ")
